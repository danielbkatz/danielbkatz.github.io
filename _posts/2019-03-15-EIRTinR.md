...
where $\beta_1$ is the regression coefficient for 1. $X_1$ takes on a value of 1 if the student is in the treatment group. Error is basically person specific (drawn from a normal population distribution).
...
$$Log[Pr(X=1|\theta_p, \delta_i)] = \beta_1*treat + \beta_2*proflevel + \theta - \delta_i$$
