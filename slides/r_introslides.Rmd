...
+ `dplyr` for creating common, easy to use and remember verbs for basic data handling tasks.
+ `ggplot2` for data vizualization 
+ `tidyr` for creating `tidy` data
+ `readr` for reading in "rectangular" data of different formats
+ `purrr` for really working with lots of data easily (for cleaning, computing variables, etc.)
+ `tibble` to make dealing with dataframes even easier. Dataframes will become tibbles in R
+ `stringr` for working with strings and doing a but of text mining
...
3. Make sure it's saved in your home directory (you'll have to name it)

4. To run a line of code in a script, put your cursor on that line, and run it, via ctrl/cmd + enter. Or Hit the `run` button in the top right of the script area
...
---
## Variable Assignment
1. One of the most fundamental aspects of R (really any language) assigning `variables` or `objects`. This is giving something a name so you can store it and work with it in R.
