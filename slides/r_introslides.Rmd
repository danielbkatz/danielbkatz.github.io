...
--
.pull-right[I'm showing you a *TON* and I don't expect you to have it all at your fingertips. But you'll know where to start, how to get help when you don't.
...
+ In R speak, the particular thing that goes in the parentheses of a function, such as `mean(x)` is called an <mark>argument</mark>

+ R functions often "take more than one argument"

+ Because of <mark>code completion</mark>, you can see what arguments a function takes as you start typing.R is making suggestions. 

+ Hit tab to select these options!
]

.pull-right[Don't worry about what this means just yet, but:
+ `ggplot()` is a function, it performs some operation

+ It takes several arguments such as `ggplot(data=df, aes(x=xvar, y=yvar))`

+ Note: `df` is often R standin speak for a hypothetical `dataframe`, and R word for dataset
...
+ When you see that an argument as an option with an equal sign, it usually means order doesn't matter as long as you type the argument first

+ For instance to perform linear regression I can type `lm(data=x, outcome~var1+var2)` Or `lm(outcome~var1+var2, data)`

+ You usually need commas between arguments
...
5. Write two lines of code (such as `1+1` and `2+2`)]
...
+ If I have a folder called, `analysis` in `thesis` - thesis is my working- directory the path will be <mark>analysis/filename.ext</mark>.
...
.center[###If you've made any comments or would like to save your current script, please go ahead an do-so now (file > save > where you want to save it > name)]
...
---
## Getting help
+ First and foremost, you can see you packages in the bottom-right pane:
...
4. As a heads up, when we start assigning variables, note what changes in your R session (where do you see the variables go?)
...
---
## Check your Understanding (classes of vectors)
...
4. You can always use `class()` to figure out the data-type of a <mark>vector</mark>.
