...
+ If for instance, we take the first row, 
```{r echo=FALSE}
knitr::kable(xtable::xtable(co16_rem[1,]),  format = "html")
...
+ It refers to the totals relevant to the state-identified `Subgroup`, `All`. 
  + According to the codebook, here: [_**California Department of Education**_](https://www.cde.ca.gov/ds/sd/sd/filescohort.asp), that's an aggregation of those particular subgroups.
  
+ However, `Subgrouptype` is 9, which refers to: `9 – Two or more races, not Hispanic`

+ So that row refers to district totals for all students who have been identified as `Two or more races, not Hispanic` but not broken up by program-participation, Gender (you really get to see how the state of CA thinks)
...
2. The variable AggLevel has information on level of analysis where
...
1. Now, using a pipe and assigning a new variable name, filtdf1000 (for exercise, not necessary, though), start with filtdf filter such that filtdf1000 only has districts with _at least_ 1000 students in its cohort (you'll need the variable . 
