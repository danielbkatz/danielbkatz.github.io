...
+ If I want to rename a column in the process of selecting the format is:
  `newdf <- old_df %>% 
  select(var1, var2, newname=oldname)`
...
1. Starting with co16_rem, edit the code from the previous slide so that you use a pipe operator, and select all variables listed in the table below.
...
1. Using `select()`, edit your code from above to add the variable below while  rename=ing them 
  + the variable, `Cohort Graduation Rate` to `gradrate`
  + Rename the variable `Cohort Dropout Rate` to `droprate`
  + Rename the variable `GED Rate` to `GEDrate`
...
+ These are just a few examples, if it can be evaluated with a logical, you can use select_if
---
## Names

+ You can rename files using names(df)[column_numbers] <- c("name for first column", "name for second column")
...
names(co16_test)[1:5] <- c("ID", "Title", "Stateleveltoschoollevel", "Charter", "Classification")
...
```{r}
co16_test <- co16_rem %>% 
  rename("NEWCODE"=CDS, "Title"=Name, "Level"=AggLevel)
...
+ Let's say I want to check if my cohort and enrollment numbers match the graduation rate of the state. 
